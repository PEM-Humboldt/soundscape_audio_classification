...
Positive samples where detected manually from audio libraries and few field samples

"""
# Set variables for positive samples
path_annot = '/Volumes/lacie_macosx/github/soundclim_annotations/bounding_boxes/Boana prasina/'  # location of bbox annotations
...
# Select only clear vocalizations
...
#%% Get spectrograms of fixed length
for idx_row, roi_fmt in rois_fmt.iterrows():
    print(idx_row, '/', len(rois_fmt))
    ...
    Sxx, tn, fn, ext = sound.spectrogram(s_trim, fs=target_fs, nperseg=spectro_nperseg,
                                         noverlap=spectro_noverlap)
    Sxx = util.power2dB(Sxx, db_range=60, db_gain=20)
    # Save spectrograms
    Sxx = np.flipud(Sxx)
    fname_save = path_save+roi_fmt.label+'_'+str(idx_row).zfill(3)
    io.imsave(fname_save+'.jpg', Sxx)#%%  
    sf.write(fname_save+'.wav', s_trim, target_fs)
...
for idx_row, roi_fmt in rois_fmt.iterrows():
    ...
    # Save spectrograms
    Sxx = np.flipud(Sxx)
    fname_save = roi_fmt.label+'_'+str(idx_row).zfill(3)
    io.imsave(path_save+fname_save+'.jpg', Sxx)#%%  
    sf.write(path_save+fname_save+'.wav', s_trim, target_fs)
